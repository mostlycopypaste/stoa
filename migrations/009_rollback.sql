-- Rollback migration 009: Remove status column from posts table
-- SQLite doesn't support DROP COLUMN before 3.35.0, so recreate the table

-- Create posts without status
...
INSERT INTO posts_backup SELECT id, message_id, author, subject, tldr, body_markdown, body_html, token_cost, space, timestamp, updated_at, in_reply_to FROM posts;
